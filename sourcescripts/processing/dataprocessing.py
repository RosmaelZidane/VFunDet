"""Make sure you run this fine from sourcescripts: example sourcescripts python3
./processing/dataprocessing.py"""
...
def _c2dhelper(item):
    ...
    savedir = utls.get_dir(utls.cache_dir() / item["dataset"] / "gitdiff")
    savepath = savedir / f"{item['id']}.git.pkl"
    if os.path.exists(savepath):
        return
    if item["func_before"] == item["func_after"]:
        return
    ret = utls.code2diff(item["func_before"], item["func_after"])
...
def get_codediff(dataset, iid):
    ...
    savedir = utls.get_dir(utls.cache_dir() / dataset / "gitdiff")
...
def bigvul(minimal=True, sample=False, return_raw=False, splits="default"):
    ...
    if minimal:
        try:
            ...
            md = pd.read_csv(utls.cache_dir() / "bigvul/bigvul_metadata.csv")
            md.groupby("project").count().sort_values("id")

            default_splits = utls.external_dir() / "bigvul_rand_splits.csv"
            ...
            if "crossproject" in splits:
                ...
                trid, vaid = utls.train_test_split(nonproject, test_size=0.1, random_state=1)
            ...
            pass
    filename = "sample_data.csv" if sample else "ProjectKB_domain_csv.csv"
    ...
    utls.dfmp(df, _c2dhelper, columns=cols, ordr=False, cs=300)

    # Assign info, remove empty space and save
    df["info"] = utls.dfmp(df, allfunc, cs=500)
    ...
    # Make splits
    df = utls.train_val_test_split_df(df, "id", "vul")
    ...
    df[metadata_cols].to_csv(utls.cache_dir() / "bigvul/bigvul_metadata.csv", index=0)
...
def assign_line_num_to_local(nodes, edges, code):
    ...
    for k, v in twohop_labels.items():
        ...
        # print(f"k = {k} v = {v}")
        # to overcome the issue with incorrect assumption, run the first time with the default code, then collect all types
        # print in the terminal, add in the list as below, comments the default fine with types. and run with the added line.
        
        # deafualt code
        
        types = [i for i in v if i in id2name and i <1000]
        
        # # new types to skip incurect assumption
        # Bibvul_wrong_list = [521,734,425,752,366,781,724,366,325,785,136,334,556,754,734,724,244,264,325,568,
        #                      781,734,686,785,679,724,508,460,299,642,799,425,136,963,668,227,499,861,990,434,679,227,861,326]
        
        # types = [i for i in v if i in id2name and i != i for i in Bibvul_wrong_list]
...
def get_node_edges(filepath, verbose=0):
    ...
    filepath = "./storage/processed/bigvul/before/53.c"
    """
...
def helper(row):
    ...
    """
    before_path = str(utls.processed_dir() / f"bigvul/before/{row['id']}.java")
    after_path = str(utls.processed_dir() / f"bigvul/after/{row['id']}.java")
    try:
...
def get_dep_add_lines_bigvul(cache=True):
    ...
    saved = utls.get_dir(utls.processed_dir() / "bigvul/eval") / "statement_labels.pkl"
    ...
    lines_dict = utls.dfmp(df, helper, ["id", "removed", "added"], ordr=False, desc=desc)
